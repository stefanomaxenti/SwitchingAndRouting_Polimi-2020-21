...
class ZodiacSwitch(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        status = threading.Timer(self.time_interval, self.check_status)
        # start timer for monitoring links and switches status
        status.start()
    ...
    #    self.logger.debug("OFPBarrierReply received %s", ev)
    ...
    def _packet_in_handler(self, ev):
        ...
        switches = [switch.id for switch in self.switches]
        ...
        if ip4_pkt:
            ...
            if self.stop == 0:
                try:
                    ...
                    self.logger.info(" --- Installing rules on path: %s", path)
    ...
    def get_topology_data(self, ev):
        self.logger.error("A new switch is active")
    ...
    def link_del_handler_reactive(self, ev):
        try:
            process = Thread(target=self.link_del_handler_thread, args=[ev])
            process.start()
        except:
            self.logger.error("Impossible to start thread")

    def link_del_handler_thread(self, ev):
        #self.logger.error("esco ciao")
        dpid_src = get_datapath(self, dpid=ev.link.src.dpid)
        dpid_dst = get_datapath(self, dpid=ev.link.dst.dpid)
        #self.logger.error("chiusura")
        link = ev.link
        reply = None
        # removing the link also from the link status list
        ...
        #    self.logger.error("Link was already removed")
         #   if self.net.has_edge(ev.link.src.dpid, ev.link.dst.dpid):
        ...
        if dpid_src is not None and dpid_dst is not None:
            reply = self.get_flows(dpid_src, ev.link.src.port_no)[0]
            # self.logger.error("Fatto")
        else:
            if dpid_src is None:
                reply = self.get_flows(dpid_dst, ev.link.dst.port_no)[0]
                # self.logger.error("Fatto")
                if ev.link.src.dpid in self.net:
                    self.net.remove_node(ev.link.src.dpid)
            if dpid_dst is None:
                reply = self.get_flows(dpid_src, ev.link.src.port_no)[0]
                # self.logger.error("Fatto")
                if ev.link.dst.dpid in self.net:
                    self.net.remove_node(ev.link.dst.dpid)
        ...
        # if dpid_src is None and link.src.dpid in self.port_occupied:
        #	self.port_occupied.pop(link.src.dpid)
        for flow in reply.body:
            ...
            if ipv4_src is not None and ipv4_dst is not None:
                ...
                for datapath in switches:
                    if datapath.id == ev.link.dst.dpid:
